...
- `ENTRYPOINT` и `CMD` - нужны для того чтобы определить как запускать нашу программу при старте контейнера, например для нас нужно будет `CMD ["/script.sh"]` (нужно брать в скобки такие чтобы напрямую запускать скрипт, а не через sh или bash)
