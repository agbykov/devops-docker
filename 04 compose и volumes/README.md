...
В секции volumes (томов), в которой указали что в той же папке что запускаем compose мы хотим создать папку `postgres_data` которая будет доступна внутри контейнера по пути `/var/lib/postgresql/data` (путь где postgres сохраняет данные по умолчанию)
